...
def lambda_handler(event, context):
    ...
    query = """
    SELECT GAME_IS_PLAYED.PERIOD_ID, TOPIC, GAME_IS_PLAYED.SCORE, GAME_PERIOD.DAYS_REMAINING, TO_CHAR(GAME_PERIOD.CREATION_TIMESTAMP, 'YYYY/MM/DD') AS START_DATE, TO_CHAR(GAME_PERIOD.CREATION_TIMESTAMP+GAME_PERIOD.DAYS_REMAINING, 'YYYY/MM/DD') AS END_DATE 
    FROM GAME_IS_PLAYED JOIN GAME_PERIOD ON GAME_IS_PLAYED.PERIOD_ID = GAME_PERIOD.PERIOD_ID
    WHERE GAME_IS_PLAYED.PLAYER_NAME = '""" + event['userName'] + """' AND GAME_PERIOD.DAYS_REMAINING = 0
    """
